...
os_system('python setup.py develop')
os_system('pip install realesrgan')
...
class Predictor(BasePredictor):
    ...
    def setup(self):
        ...
        half = True if torch_cuda.is_available() else False
    ...
    def predict(
        ...
        try:
            extension = os_path.splitext(os_path.basename(str(img)))[1]
            ...
            out_path = Path(tempfile_mkdtemp()) / f'out.{extension}'
...
def clean_folder(folder):
    for filename in os_listdir(folder):
        file_path = os_path.join(folder, filename)
        try:
            if os_path.isfile(file_path) or os_path.islink(file_path):
                os_unlink(file_path)

...
class ExecuteCommand:
    ...
    def __restore_btn_state(self):
        self.main_frame_control.execute_btn.config(
            text="开 始 处 理", bg=self.main_frame_control.execute_btn.get_dynamic_style["bg"],
    ...
    def __execute_in_cmd(self):
        process = Popen('GFPGAN.exe', stdout=PIPE, stderr=PIPE, shell=True, text=False)
        self.main_frame_control.execute_btn.config(state='normal', command=self.set_stop_signal)
        for line in iter(process.stdout.readline, b''):
            try:
                ...
                if process.poll() is not None or self.__stop:
                    break
            except Exception as e:
                with open("Data/Error_log.txt", 'a', encoding='utf-8')as f:
                    f.write(f"{e}\n")
                self.main_frame_control.overwrite_to_show_label(f'出现异常, 详见[帮助]中的[错误日志]!')

...
from FileManager import FileJudgement, SpgManager, FileOperation
...
class GuiControl(MainFrame):
    ...
    def update_widget_color(self):
        ...
        widgets = (self.label1, self.label2, self.label3, self.label4, self.label5, self.label6,
                   self.open_folder1, self.open_folder2, self.left_btn, self.right_btn,
                   self.infile_path_entry, self.outfile_path_entry, self.choose_folder1, self.choose_folder2,
                   self.execute_btn, self.img_label1, self.img_label2, self.task_list, self.tip_entry,
                   self.orig_img_info_entry, self.processed_img_info_entry, self.up_task_btn, self.down_task_btn,
                   self.task_clear_btn, self.remove_task_btn, self.refresh_task_btn, self.progressbar)
        for widget in widgets:
            widget.config(widget.get_dynamic_style)
        # second part
        ImageLabel.overwrite = True
        self.parent.config(bg=Theme.win_bg)
        self.style_config.re_config()
        # change the stuff_obj's color
        self.h_seperator[2].config(highlightbackground=Theme.scrollbar_bg)
        self.up_scale_slider.config(bg=Theme.win_bg, foreground=Theme.main_fg,
    ...
    def bind_command(self):
        ...
        for button, command in zip(theme_buttons, commands):
            button.config_command(command)
        self.left_btn.config(command=self.page_manager.turn_left)
        self.right_btn.config(command=self.page_manager.turn_right)
        self.choose_folder1.config(command=lambda: self.choose_folder(True))
        self.choose_folder2.config(command=lambda: self.choose_folder(False))
        self.open_folder1.config(command=lambda: self.start_folder(True))
        self.open_folder2.config(command=lambda: self.start_folder(False))
    ...
    def start_file(self, event):
        ...
        if isinstance(file_path, int):
            return
        Thread(target=FileOperation.start_file(file_path)).start()
    ...
    def main(self):
        if FileJudgement.is_folder_name_error(self.infile_path_entry.get(), self.outfile_path_entry.get(),
